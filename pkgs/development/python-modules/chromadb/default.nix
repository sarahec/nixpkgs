...
buildPythonPackage rec {
  ...
  src = fetchFromGitHub {
    ...
    repo = "chroma";
    tag = version;
    hash = "sha256-WxJW9u7PMEmyc4WjeGbLHq8VG7p55Ko8v6ObgPFSChE=";
  };

  cargoDeps = rustPlatform.fetchCargoVendor {
    inherit src;
    name = "${pname}-${version}";
    hash = "sha256-QNCRalJZtTWk2LpPfVbXkX3jEHwfKMgA0O3NGS/Fa5E=";
  };

  # Can't use fetchFromGitHub as the build expects a zipfile
  swagger-ui = fetchurl {
    url = "https://github.com/swagger-api/swagger-ui/archive/refs/tags/v5.21.0.zip";
    hash = "sha256-mF2/fdZm4cPs045xiAtG+Q2qW716jUpLw9IwBdJTR8M=";
  };

  patches = [
    # The fastapi servers cant set up their networking in the test environment, so disable for testing
  ...
  disabledTests = [
    ...
    # These either have https xonnection errors or name resolution errors
  ...
  meta = {
    ...
    mainProgram = "chroma";
    # likely not broken at 1.x, but need someone with access to investigate
